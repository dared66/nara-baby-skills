...
- Keep the returned activity ID. Edits and new activities submit the full record to the sync queue and verify it through fresh history, avoiding the direct RTDB creation path that returned HTTP 401. Bottle and diaper commands retain deterministic activity IDs. Queue acceptance alone does not establish that the activity was saved. If a request fails after submission, read back the known ID before any retry. Never blindly rerun a create with a new ID. If the result cannot be determined, report the uncertain outcome and stop further writes.

...
- Upstream `_push_payload` performs two writes. The adapter replaces both `log_activity` and `patch_activity` with a single sync-queue submission, avoiding direct RTDB creation (which returned HTTP 401). New records retain the supplied activity ID and validated timezone; edits use the full existing record from fresh sync. Neither path creates a legacy RTDB shadow. A queue response alone is not verified persistence; the bottle and diaper commands check fresh history before reporting success. See Sync-backed edits below.
